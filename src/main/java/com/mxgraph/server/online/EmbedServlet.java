...
public class EmbedServlet {
        ...
        if (sparam != null) {
            ...
            for (int i = 0; i < names.length; i++) {
                if (names[i].indexOf("..") < 0 && !done.contains(names[i])) {
                    if (names[i].equals("*")) {
                        js.append(readXmlFile("/js/shapes.min.js", false, request));
                        result.append(
                                "'" + readXmlFile("/stencils.xml", true, request) + "'");
                    } else {
    ...
    public String readXmlFile(String filename, boolean xmlContent, HttpServletRequest request)
            throws IOException {
        String result = readFile(filename, request);
    ...
    public String readFile(String filename, HttpServletRequest request) throws IOException {
